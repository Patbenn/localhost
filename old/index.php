...
<?php
			            foreach ( $devtools as $tool ) {
			            	printf( '<li><a href="%1$s" target="_NEW">%2$s</a></li>', $tool['url'], $tool['name'] );
			            }
?>
			        </ul>
			    </nav>
...
<?php
		    foreach ( $dir as $d ) {
				...
				printf( '<h3>BIStrainer Projects</h3><ul class="sites %1$s">', $dirname );
		        ...
		        foreach( glob( $d ) as $file )  {
		        	...
		        	if ( !in_array( $project, $bistrainer ) ) continue;
...
<?php
		    foreach ( $dir as $d ) {
				...
				printf( '<br style="clear:both;" /><h3>Learning Projects</h3><ul class="sites %1$s">', $dirname );
		        ...
		        foreach( glob( $d ) as $file )  {
		        	...
		        	if ( !in_array( $project, $learning ) ) continue;
...
<?php
		    foreach ( $dir as $d ) {
				...
				printf( '<br style="clear:both;" /><h3>Designs to Steal</h3><ul class="sites %1$s">', $dirname );
		        ...
		        foreach( glob( $d ) as $file )  {
		        	...
		        	if ( !in_array( $project, $ideas ) ) continue;
